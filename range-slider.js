class RangeSlider {
	...
	setValue(values) {
		let safeValues = this.points.map((point, index) => {
			let value = this.safeValue(values[index]);
			this.positionPoint(point, value);
			return value;
		});
	...
	handleDrag(e) {
		...
		let dir = moveX > 0 ? 1 : -1;
		let newValue = this.safeValue(this.movePoint.value + increment * dir);
	...
	renderPoints() {
		this.points = this.opts.value.map((v, index) => {
			...
			this.positionPoint(pointNode, this.safeValue(v));
